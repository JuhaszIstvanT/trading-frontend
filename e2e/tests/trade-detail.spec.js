...
describe("Wallet Component E2E Test", () => {
  ...
  it("should place a stop-loss order successfully", async () => {
    const firstTradeRow = await driver.wait(
      until.elementLocated(By.css("app-trade .trade-history .trade-row")),
      10000
    );
    ...
    const stopLossOrderInput = await driver.wait(
      until.elementLocated(By.id("stopLossOrderPrice")),
      10000
    );
    await stopLossOrderInput.clear();
    await stopLossOrderInput.sendKeys("1000");

    const placeStopLossOrderButton = await driver.findElement(
      By.css("button.btn-danger")
    );
    await driver.executeScript(
      "arguments[0].scrollIntoView(true);",
      placeStopLossOrderButton
    );
    await placeStopLossOrderButton.click();
